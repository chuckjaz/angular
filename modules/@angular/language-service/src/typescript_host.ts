...
import {CompileDirectiveMetadata, CompilerConfig, createOfflineCompileUrlResolver} from '@angular/compiler';
import {extractProgramSymbols} from '@angular/compiler-cli/src/codegen';
import {StaticReflector, StaticSymbol} from '@angular/compiler-cli/src/static_reflector';
import {DirectiveNormalizer} from '@angular/compiler/src/directive_normalizer';
import {DirectiveResolver} from '@angular/compiler/src/directive_resolver';
import {CompileMetadataResolver} from '@angular/compiler/src/metadata_resolver';
import {NgModuleResolver} from '@angular/compiler/src/ng_module_resolver';
import {NgAnalyzedModules, analyzeNgModules} from '@angular/compiler/src/offline_compiler';
  ...
  getAnalyzedModules(): NgAnalyzedModules {
    ...
    if (!analyzedModules) {
      const programSymbols =
          extractProgramSymbols(this.program, this.reflector, this.reflectorHost, {
            genDir: '',
            basePath: '',
      ...
      analyzedModules = this.analyzedModules =
          analyzeNgModules(programSymbols, {transitiveModules: true}, this.resolver);
    }
      ...
      case this.ts.SyntaxKind.StringLiteral:
        ...
        if (declaration) {
          ...
          return this.getSourceFromDeclaration(
              ...
              this.reflectorHost.getStaticSymbol(sourceFile.fileName, declaration.name.text),
      ...
      for (const decorator of node.decorators) {
        if (decorator.expression && decorator.expression.kind == ts.SyntaxKind.CallExpression) {
          ...
          if (classDeclaration.name) {
            ...
            if (type) {
              const staticSymbol = this._reflectorHost.getStaticSymbol(
                  sourceFile.fileName, classDeclaration.name.text);
              try {
