...
export class ReflectorHost implements StaticReflectorHost, ImportGenerator {
  ...
  getImportPath(containingFile: string, importedFile: string) {
    ...
    // If a file does not yet exist (because we compile it later), we still need to assume
    // it exists it so that the `resolve` method works!
  ...
  findDeclaration(
    ...
    try {
      ...
      if (!sf || !(<any>sf).symbol) {
        // The source file was not needed because no exported symbol needed a reference
        // to the file so TypeScript elided the reference from the .d.ts file; but the
        // values from the .metadata.json file are being requested which is stored
        // along side the file containing the original declaration of the symbol.

        if (EXPLICIT_PATH.test(module)) {
          // For explicit paths (such as an absolute or relative path) just assume the
          // origiinal declaration is in the file location referenced. This is an
          // assumption to save a significant amount of time as creating a program is
          // expensive.
          return this.getStaticSymbol(filePath, symbolName);
        } else {
          // To find the original declaration, create a new program based on the current
          // program that has the resolved module added as a root file.
          const roots = program.getRootFileNames().concat(filePath);

          this.program = program =
              ts.createProgram(roots, program.getCompilerOptions(), null, program);
          tc = program.getTypeChecker();
          sf = program.getSourceFile(filePath);

          if (!sf || !(<any>sf).symbol) {
            throw new Error(`Cannot find source file for module ${module}`);
          }
        }
      }
