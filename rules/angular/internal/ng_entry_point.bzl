...
load(":ng_library.bzl", "AngularMetadataOutput", "ModuleId")

AngularEntryPointOutput = provider()
...
def _ngc_tsconfig(ctx, files, srcs, apis, index_name, **kwargs):
  config = tsc_wrapped_tsconfig(ctx, files, srcs, AngularEntryPointOutput, **kwargs)
  return dict(config, **{
...
def _ng_entry_point_impl(ctx):
  ...
  metadata_files = _collect_files(ctx, AngularMetadataOutput)
  ...
  return struct(
    ...
    providers = [
      AngularEntryPointOutput(index = struct(
        js_file = index_js_file,
        dts_file = index_dts_file,
        metadata_file = index_metadata_file,
      ))
    ]
  )
...
ng_entry_point = rule(
    ...
    attrs = {
      ...
      # Dpulicated from the ng_module rule
      "compiler": attr.label(
