...
import {preparseElement, PreparsedElementType} from '../template_parser/template_preparser';
...
export class HtmlToTemplateTransform implements html.Visitor {
  ...
  visitElement(element: html.Element): t.Node | null {
    ...
    for (const attribute of element.attrs) {
      const hasBinding = this.parseAttribute(isTemplateElement, attribute, matchableAttributes,
        boundProperties, boundEvents, variables, references);
      ...
      if (normalizedName.startsWith(TEMPLATE_ATTR_PREFIX)) {
        templateBindingsSource = attribute.value;
    ...
    const children: t.Node[] = html.visitAll(
    ...
    if (preparsedElement.type === PreparsedElementType.NG_CONTENT) {
      ...
      // TODO(chuckj): Should the attributes be matchableAttributes instead? This currently ignores all bound properties.
      parsedElement = new t.Content(attributes, element.sourceSpan);
      ...
      parsedElement = new t.Element(
        element.name, attributes, boundAttributes, boundEvents, children, references, element.sourceSpan,
    ...
    if (hasInlineTemplate) {
      ...
      parsedElement = new t.Template(
        attributes, boundAttributes, [parsedElement], [], templateVariables, element.sourceSpan, element.startSourceSpan, element.endSourceSpan);
    }
  ...
  visitText(text: html.Text): t.Node {
    ...
    return expr ? new t.BoundText(expr, text.sourceSpan) :
  ...
  private createBoundAttributes(elementName: string, boundProperties: BoundProperty[]): t.BoundAttribute[] {
    return boundProperties.filter(v => !v.isLiteral).map(property => {
      const oldElement = this.bindingParser.createElementPropertyAst(elementName, property)
      return new t.BoundAttribute(oldElement.name, oldElement.type as any as t.PropertyBindingType, oldElement.securityContext,
        oldElement.value, oldElement.unit, oldElement.sourceSpan);
    });    
  }
  
  private parseAttribute(isTemplateElement: boolean, attribute: html.Attribute,
    matchableAttributes: string[][], boundProperties: BoundProperty[], 
    boundEvents: t.BoundEvent[], variables: t.Variable[],
...
class NonBindableVisitor implements html.Visitor {
  visitElement(ast: html.Element): t.Element|null {
    ...
    return new t.Element(
        ...
        /* inputs */ [], /* outputs */ [], children,  /* references */[], 
