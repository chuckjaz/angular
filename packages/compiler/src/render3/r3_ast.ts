...
import {BoundPropertyType} from '../template_parser/binding_parser';
...
export class BoundAttribute implements Node {
  constructor(
    public name: string, public type: PropertyBindingType, public securityContext: SecurityContext, public value: AST, public unit: string|null,
...
export class BoundEvent implements Node {
  constructor(
    public name: string, public handler: AST, public target: string|null, public phase: string|null,
    public sourceSpan: ParseSourceSpan) {}
...
export class Element implements Node {
  constructor(
      public name: string, public attributes: TextAttribute[],
...
export class TransformVisitor implements Visitor<Node> {
  ...
  visitTemplate(template: Template): Node { 
    ...
    if (newAttributes != template.attributes || newInputs != template.inputs || newChildren != template.children || newVariables != template.variables || newReferences != template.references) {
  ...
  visitContent(content: Content): Node {
    const newAttributes = transformAll(this, content.attributes);
    if (newAttributes != content.attributes) {
      return new Content(newAttributes, content.sourceSpan);
    }
    return content;
  }
...
export function transformAll<Result extends Node>(visitor: Visitor<Node>, nodes: Result[]): Result[] {
  const result: Result[] = [];
  let changed = false;
  for (const node of nodes) {
    const newNode = node.visit(visitor);
