...
import { CompileDirectiveMetadata, CompileQueryMetadata, flatten, sanitizeIdentifier, CompileTypeMetadata, tokenReference } from '../compile_metadata';
import { CompileReflector } from '../compile_reflector';
import { BindingParser } from '../template_parser/binding_parser';
import { OutputMode, BUILD_OPTIMIZER_COLOCATE } from './r3_types';
import { identifierName, PropertyBindingType } from '../compiler';
import { CssSelector } from '../selector';
import { ConstantPool, DefinitionKind } from '../constant_pool';
import { LifecycleHooks } from '../lifecycle_reflector';
import { LocalResolver, BindingForm, convertPropertyBinding, convertActionBinding, BuiltinFunctionCall } from '../compiler_util/expression_converter';
...
export function compileComponent(
    ...
    template: t.Node[], reflector: CompileReflector, bindingParser: BindingParser,
    mode: OutputMode) {
  const definitionMapValues: {key: string, quoted: boolean, value: o.Expression}[] = [];
...
function unknown<T>(arg: o.Expression | o.Statement | t.Node): never {
  throw new Error(
...
class TemplateDefinitionBuilder implements t.Visitor<void>, LocalResolver {
  ...
  private _contentProjections: Map<t.Content, NgContentInfo>;
  ...
  private _phToNodeIndexes: {[phName: string]: number[]}[] = [{}];
  ...
  constructor(
      ...
      private viewQueries: CompileQueryMetadata[]) {
        ...
        outputCtx, () => this.allocateDataSlot(), (name, localName, slot, value) => {
          ...
          this._creationMode.push(
              o.importExpr(R3.pipe)
                  .callFn([
                    o.literal(slot), o.literal(name)
                  ])
                  .toStmt());
        });
  }

  buildTemplateFunction(nodes: t.Node[], variables: t.Variable[]): o.FunctionExpr {
    ...
    // TODO(chuckj): Content projection
    ...
    for (const phToNodeIdx of this._phToNodeIndexes) {
  ...
  visitContent(content: t.Content) {
    const info = this._contentProjections.get(content) !;
    info || error(`Expected ${content.sourceSpan} to be included in content projection collection`);
    const slot = this.allocateDataSlot();
    const parameters = [o.literal(slot), o.literal(this._projectionDefinitionIndex)];
    if (info.index !== 0) {
      parameters.push(o.literal(info.index));
    }
    this.instruction(this._creationMode, content.sourceSpan, R3.projection, ...parameters);
  }
  ...
  visitElement(element: t.Element) {
    ...
    if (this._inI18nSection) {
      ...
      if (!this._phToNodeIndexes[this._i18nSectionIndex][phName]) {
        this._phToNodeIndexes[this._i18nSectionIndex][phName] = [];
      }
      this._phToNodeIndexes[this._i18nSectionIndex][phName].push(elementIndex);
    }
    ...
    for (const attribute of element.attributes) {
      ...
      if (name === I18N_ATTR) {
        ...
        this._phToNodeIndexes[this._i18nSectionIndex] = {};
        i18nMeta = value;
  ...
  visitTemplate(ast: t.Template) {
    ...
    const templateName = `Template_${templateIndex}`;
    ...
    const templateVisitor = new TemplateDefinitionBuilder(
        ...
        this.bindingScope.nestedScope(), this.level + 1, this.ngContentSelectors, null,
        null, []);
  ...
  visitBoundText(text: t.BoundText) {
    ...
    // Refresh mode
    this.instruction(
        this._refreshMode, text.sourceSpan, R3.textCreateBound, o.literal(nodeIndex),
        this.bind(o.variable(CONTEXT_NAME), text.value, text.sourceSpan));
  }
