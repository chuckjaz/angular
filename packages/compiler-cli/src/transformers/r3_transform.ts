...
export type TransfomerFactory = (context: ts.TransformationContext) => Transformer;

/**
 * Returns a transformer that adds the requested static methods specified by modules.
 */
export function getAngularClassTransfomerFactory(modules: PartialModule[]): TransfomerFactory {
  if (modules.length === 0) {
